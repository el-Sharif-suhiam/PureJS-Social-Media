import { default as axios } from "axios";
  ...
  let profileContainer = document.getElementById("profile-cards-container")
...
function loader(condition) {
  ...
  function fillData(){
  const profileBackIMG = document.getElementById("profile-backIMG");
  const profileIMG = document.getElementById("profileIMG");
  const profileName = document.getElementById("prof-name");
  const profileID = document.getElementById("prof-id");
  const profilePostNum = document.getElementById("postNum");
  const profileCommentNum = document.getElementById("commentNum");
  axios.get(`${baseUrl}/users/${userId}`)
  .then( respone => {

    const userData = respone.data.data;
    profileName.innerText = userData.name;
    profileID.innerText = `@${userData.username}`
    profilePostNum.innerText = userData.posts_count;
    profileCommentNum.innerText = userData.comments_count;
    if(String(userData.profile_image) === "[object Object]" ){
      profileIMG.src = "Images/blank-person.jpeg"
      profileBackIMG.src = "Images/Blank-Background.jpeg"
    }else{
    profileIMG.src = userData.profile_image;
    profileBackIMG.src = userData.profile_image
    }
  })
}
fillData()
...
window.addEventListener("scroll", function () {
  ...
  setTimeout(function () {
    if (endPage && currentPage < lastPage) {
      currentPage += 1;
      userPosts(currentPage, false);
    }
  }, 1000);
});

function userPosts(page = 1, reload = true ) {
  loader(true)
  axios.get(`${baseUrl}/users/${userId}/posts`)
  .then((respone) => {
    //let lastPage = respone.data.meta.last_page;
    let token = localStorage.getItem("Sh-Token");
    const user = JSON.parse(localStorage.getItem("Sh-user"))
    const responeData = (respone.data.data).reverse();
    ...
    for (post of responeData) {
      ...
      if(user !== null && author.id === user.id) {
        card = `<div class="card my-4 mx-5 shadow">
        ...
        <div class="card-header">
          <img src="${profileImage}" alt="person icon" class="rounded-circle user-icon me-3">
          <b class="user-id">${author.username}</b>
        </div>
        ${postImage}
        <span class="created-at mx-3 mt-1">${post.created_at}</span>
        <div class="card-body">
